...
public class MainJFrame extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        userNameJTextField.setBackground(new java.awt.Color(0, 153, 255));
...
//            System.out.println(system.getMedicalBoothDirectory());
            UserAccountMedicalBooth userAccountMedicalBooth = system.getMedicalBoothUADirectory().authenticateUser(userName, password);
            ...
            if (userAccountMedicalBooth == null) {
                //Step 2: Go inside each network and check each enterprise
                for (Network network : system.getNetworkList()) {
                    //Step 2.a: check against each enterprise
                    for (Enterprise enterprise : network.getEnterpriseDirectory().getEnterpriseList()) {
                        userAccountMedicalBooth = enterprise.getMedicalBoothUADirectory().authenticateUser(userName, password);
                        if (userAccountMedicalBooth == null) {
                            //Step 3:check against each organization for each enterprise
                            for (Organization organization : enterprise.getOrganizationDirectory().getOrganizationList()) {
                                userAccountMedicalBooth = organization.getMedicalBoothUADirectory().authenticateUser(userName, password);
                                if (userAccountMedicalBooth != null) {
            ...
            if (userAccountMedicalBooth == null && userAccountFoodSupplier == null && userAccountMedicalSupplier == null) {
                JOptionPane.showMessageDialog(null, "Invalid credentials");
                return;
            } else {
                if (userAccountMedicalBooth != null) {
                    CardLayout layout = (CardLayout) container.getLayout();
                    container.add("workArea", userAccountMedicalBooth.getRole().createWorkArea(container, userAccountMedicalBooth, inOrganization, inEnterprise, system));
